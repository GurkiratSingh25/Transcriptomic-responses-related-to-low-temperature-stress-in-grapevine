...
#SBATCH --job-name=CoCo_counts_try10.sh_20230922
...
coco_correct_gtf_file=/fs/ess/PAS1444/Freezing_tolerance_RNA_Seq/work_dir/CoCo/Vitis_labrusca_annoV2.cococorrectannoout.gtf   # genome annotation file which is the output from the 'coco correct_annotation' command
bam_input_files=/fs/ess/PAS1444/Freezing_tolerance_RNA_Seq/work_dir/STAR/STAR_align/bam/ 										# input file directory (the real deal .bam files) in .bam format
output_location=/fs/ess/PAS1444/Freezing_tolerance_RNA_Seq/work_dir/CoCo/														# output location (just location, not the name of the output file, that comes in the for loop)
...
#This script actually places the output files at location '/fs/ess/PAS1444/Freezing_tolerance_RNA_Seq/work_dir/STAR/STAR_align/bam'
#You must therefore mv all of the output files to location '/fs/ess/PAS1444/Freezing_tolerance_RNA_Seq/work_dir/CoCo/' by use of the command line.
